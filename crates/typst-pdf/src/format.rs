...
///
/// - Which @pdf:pdf-standards[PDF standards] Typst should enforce conformance
///   with by specifying `--pdf-standard` followed by one or multiple
///   comma-separated standards. Valid standards are `1.4`, `1.5`, `1.6`, `1.7`,
///   `2.0`, `a-1b`, `a-1a`, `a-2b`, `a-2u`, `a-2a`, `a-3b`, `a-3u`, `a-3a`,
///   `a-4`, `a-4f`, `a-4e`, and `ua-1`. By default, Typst outputs
///   PDF-1.7-compliant files.
///
/// - You can disable PDF tagging completely with `--no-pdf-tags`. By default,
...
///   file inaccessible and prevent conformance with accessible conformance
...
/// specific use-cases. By default, Typst exports PDF 1.7 files. Adobe Acrobat 8
/// and later as well as all other commonly used PDF viewers are compatible with
/// this PDF version.
///
/// Some features of Typst may not be available depending on the PDF standard
/// you choose. You can combine compatible PDF/A and PDF/UA standards. Notably,
/// PDF/UA-1 and PDF/A-4 are mutually incompatible since the former requires PDF
/// 1.7 or earlier and the latter requires PDF 2.0.
///
/// == PDF versions <pdf-versions>
/// Typst supports five different PDF versions:
/// 1.4, 1.5, 1.6, 1.7 (default), and 2.0. You can choose each of these versions
/// for your document export. However, based on the features you used there may
/// be a minimum version. Likewise, the standards you target can limit which
...
/// errors when working with older hardware. For general exchange, we recommend
/// keeping the default PDF 1.7 setting or choosing PDF 2.0.
///
...
/// for _@guides:accessibility:basics[Universal Access]._ When you choose this
/// PDF standard, Typst will run additional checks when exporting your document.
/// These checks will make sure that you are following accessibility best
/// practices. For example, it will make sure that all your images come with
/// alternative descriptions.
///
/// Note that there are some rules in PDF/UA that are crucial for accessibility
/// but cannot be automatically checked. Hence, when exporting a PDF/UA-1
/// document, make sure you did the following:
///
...
///   @figure[figures], and @list[lists]) when appropriate instead of defining
///   custom constructs. This lets Typst know (and export) the role a construct
///   plays in the document. See @guides:accessibility[the Accessibility guide]
///   for more details.
/// - Do not exclusively use contrast, color, format, or layout to communicate
///   an idea. Use text or alternative descriptions instead of or in addition to
///   these elements.
...
/// provide @math:accessibility[alternative descriptions of mathematics] in
/// natural language.
///
...
/// recommend exporting to PDF/UA-1 instead of PDF 2.0 for the additional checks
/// and greater compatibility. The second part of PDF/UA is designed for PDF
/// 2.0, but not yet supported by Typst.
///
/// == PDF/A <pdf-a>
/// Typst optionally supports emitting PDF/A-conformant files. PDF/A files are
/// geared towards maximum compatibility with current and future PDF tooling.
/// They do not rely on difficult-to-implement or proprietary features and
/// contain exhaustive metadata. This makes them suitable for long-term
/// archival.
///
/// The PDF/A Standard has multiple versions (_parts_ in ISO terminology) and
/// most parts have multiple profiles that indicate the file's conformance
/// level. You can target one part and conformance level at a time. Currently,
/// Typst supports these PDF/A output profiles:
///
/// - *PDF/A-1b:* The _basic_ conformance level of ISO 19005-1. This version of
///   PDF/A is based on PDF 1.4 (2001) and results in self-contained, archivable
///   PDF files. As opposed to later parts of the PDF/A standard, transparency
///   is not allowed in PDF/A-1 files.
///
...
///   accessible _Tagged PDF_ file. Note that accessibility is improved with
///   later parts as not all PDF accessibility features are available for PDF
///   1.4 files. Furthermore, all text in the file must consist of known Unicode
///   code points.
///
/// - *PDF/A-2b:* The _basic_ conformance level of ISO 19005-2. This version of
///   PDF/A is based on PDF 1.7 (2006) and results in self-contained, archivable
///   PDF files.
///
/// - *PDF/A-2u:* This is the _Unicode-mappable_ conformance level that builds
///   on the basic level A-2b. It also adds rules that all text in the document
///   must consist of known Unicode code points. If possible, always prefer this
...
///   @guides:accessibility[Accessibility Guide] when targeting this conformance
///   level. Finally, PDF/A-2a forbids you from using code points in the
///   #link("https://en.wikipedia.org/wiki/Private_Use_Areas")[Unicode Private
///   Use area]. If you want to build an accessible file, also consider
///   additionally targeting PDF/UA-1, which enables more automatic
///   accessibility checks.
///
/// - *PDF/A-3b:* The _basic_ conformance level of ISO 19005-3. This version of
///   PDF/A is based on PDF 1.7 (2006) and results in archivable PDF files that
///   can contain arbitrary other related files as @pdf.attach[attachments]. The
///   only difference between it and PDF/A-2b is the capability to attach
///   non-PDF/A-conformant files.
///
/// - *PDF/A-3u:* This is the _Unicode-mappable_ conformance level that builds
///   on the basic level A-3b. Just like PDF/A-2b, this requires all text to
///   consist of known Unicode code points. These rules do not apply to
///   attachments. If possible, always prefer this standard over PDF/A-3b.
///
...
///   accessible _Tagged PDF_ and to not use characters from the Unicode Private
///   Use area. Just like before, these rules do not apply to attachments.
///
/// - *PDF/A-4:* The basic conformance level of ISO 19005-4. This version of
///   PDF/A is based on PDF 2.0 (2017) and results in self-contained, archivable
///   PDF files. PDF/A-4 has no parts relating to accessibility. Instead, the
///   topic has been elaborated on more in the dedicated PDF/UA standard.
///   PDF/A-4 files can conform to PDF/UA-2 (currently not supported in Typst).
///
...
///   other related files as @pdf.attach[attachments], just as files conforming
///   to part 3 of ISO 19005. The only difference between it and PDF/A-4 is the
///   capability to attach non-PDF/A-conformant files.
///
/// - *PDF/A-4e:* The _engineering_ conformance level that builds on the
///   embedded files level A-4f. Files conforming to this level can contain 3D
///   objects. Typst does not support 3D content, so this is functionally
...
/// questions:
///
/// + *Does pre-2006 software or equipment need to be able to read my file?* If
...
///   (PDF/A-4f).
///
/// + *Does your file need to use features introduced in PDF 2.0?* Currently,
///   use of PDF 2.0 features in Typst is limited to minor improvements in
///   accessibility, e.g. for the @title[title element]. If you can do without
///   these improvements, maximize compatibility by choosing part two (when you
///   don't need attachments) or part three (when you do need attachments). If
///   you rely on PDF 2.0 features, use part four.
///
/// Now, you only need to choose a *conformance level* (the lowercase letter at
/// the end).
///
/// - *If you decided on part one, two, or three,* you should typically choose
///   the accessible conformance level of your part, indicated by a lowercase
///   `a` at the end. If your document is inherently inaccessible, e.g. an
///   artist's portfolio that cannot be boiled down to alternative descriptions,
///   choose conformance level `u` instead. Only if this results in a compiler
///   error, e.g. because you used code points from the Unicode Private Use
///   Area, use the basic level `b`.
///
...
/// PDF/A files contain additional metadata, and that some readers will prevent
/// the user from modifying a PDF/A file.
///
...
///
/// This module contains some functions without a final API. They are designed
